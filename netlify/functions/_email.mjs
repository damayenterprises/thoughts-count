...
const WRAP = (inner) => `
<div style="margin:0;padding:24px;background:#f7f3ec;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#2c2a26;">
  <div style="max-width:560px;margin:0 auto;background:#fdfbf7;border:1px solid #e7ded0;border-radius:20px;overflow:hidden;">
    <div style="padding:20px 28px;border-bottom:1px solid #e7ded0;font-family:Georgia,serif;font-size:20px;font-weight:600;color:#118ab9;">&#10084; Thoughts Count</div>
    <div style="padding:26px 28px;line-height:1.6;font-size:15px;">${inner}</div>
    <div style="padding:16px 28px;border-top:1px solid #e7ded0;font-size:12px;color:#8a8377;">Helping good intentions become meaningful actions.</div>
  </div>
</div>`;

const esc = (s) => String(s == null ? "" : s).replace(/[&<>]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;" }[c]));
const H = (t) => `<div style="font-family:Georgia,serif;font-size:16px;font-weight:600;color:#0a5876;margin:18px 0 6px;">${esc(t)}</div>`;
...
export function planEmailHtml(plan) {
  ...
  const actions = (p.thoughtful_actions || [])
    .map((a) => `<div style="margin-bottom:10px;"><b>${esc(a.action)}</b><br><span style="color:#5a554c;">${esc(a.why_it_fits)} · ${esc(a.effort || "")} · ${esc(a.approx_cost || "")}</span></div>`)
    .join("");
  const gifts = (p.gift_ideas || []).length
    ? H("If you'd like to send something") + (p.gift_ideas || []).map((g) =>
        `<div style="margin-bottom:8px;"><b>${esc(g.title)}</b> — <span style="color:#5a554c;">${esc(g.price_range || "")}. ${esc(g.why_it_fits || "")}</span></div>`).join("")
    : "";
  const follow = (p.follow_up || [])
    .map((f) => `<div style="margin-bottom:6px;"><b style="color:#0a5876;">${esc(f.when)}:</b> ${esc(f.gesture)}</div>`)
    .join("");

  const inner = `
    <div style="font-family:Georgia,serif;font-size:19px;color:#2c2a26;margin-bottom:8px;">${esc(p.headline || "Your plan")}</div>
    ...
    <div style="margin-top:18px;font-style:italic;color:#5a554c;">${esc(p.closing_encouragement || "")}</div>`;
  return WRAP(inner);
}
...
export function reminderEmailHtml({ when, gesture, headline }) {
  const inner = `
    <div style="font-family:Georgia,serif;font-size:18px;color:#2c2a26;margin-bottom:6px;">A gentle nudge for ${esc(when)}</div>
    <div style="color:#5a554c;margin-bottom:14px;">You told me you wanted to keep showing up${headline ? " for " + esc(headline) : ""}, so here I am.</div>
    <div style="background:#e3f0f6;border-radius:14px;padding:16px 18px;font-size:16px;">${esc(gesture)}</div>
    <div style="margin-top:16px;color:#5a554c;">A small gesture now, when everyone else has moved on, is exactly the kind that gets remembered.</div>
    ${herSignoff()}`;
  return WRAP(inner);
}
...
export function peopleNudgeEmailHtml({ personName, label, whenText, planUrl }) {
  const inner = `
    <div style="font-family:Georgia,serif;font-size:18px;color:#2c2a26;margin-bottom:6px;">${esc(personName)}'s ${esc(label)} is ${esc(whenText)}</div>
    <div style="color:#5a554c;margin-bottom:14px;">Just a quiet heads-up from me, so you have time to show up the way you'd want to.</div>
    <div style="background:#e3f0f6;border-radius:14px;padding:16px 18px;font-size:15px;">Want to think it through together for ${esc(personName)}? <a href="${esc(planUrl)}" style="color:#118ab9;font-weight:600;text-decoration:none;">Take two minutes with me here</a></div>
    <div style="margin-top:16px;color:#5a554c;">Thinking of it ahead of everyone else is what makes it land.</div>
    ${herSignoff()}`;
  return WRAP(inner);
}
