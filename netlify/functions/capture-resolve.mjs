...
import { deleteFact } from "./_memory.mjs";
...
export default async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case "undo": {
        // Undo a Level-A save: remove the facts it wrote and drop the capture. Idempotent.
      ...
      case "reassign": {
        ...
        // Decide the target person: an explicit pick (reassign / a chosen new person) wins,
        // else the engine's proposal, else create the named new person.
        let personId = body?.personId || null;
        if (personId) {
          const p = await getPerson(supa, userId, personId);
          if (!p) return json(404, { error: "We couldn't find that person." });
        ...
        const factIds = await writeFactsToPerson(supa, userId, personId, facts, cap.source || "typed", cap.raw_text || "");
        await supa.from("captures").update({
          status: "confirmed",
          proposed_person_id: personId,
          parsed: { ...(cap.parsed || {}), written_fact_ids: factIds },
