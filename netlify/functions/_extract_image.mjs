...
const EXTRACTED_PERSON_ITEM = {
  ...
  properties: {
    ...
    event: {
      type: "object",
      description:
        "An event this artifact carries FOR THIS PERSON, when a date or occasion is literally shown. A wedding invite/save-the-date → occasion \"wedding\", the printed wedding date, recurring:false. A baby announcement → occasion \"birthday\", the birth date, recurring:true (a birthday repeats). A birthday/greeting card with a printed birth date → occasion \"birthday\", recurring:true. An obituary → on each living SURVIVOR (never the deceased, who is never returned) occasion \"loss of <deceased name>\", the passing/service date if shown, recurring:false. A business card usually carries NO event (omit). Omit the whole object if no occasion/date is shown for this person. Never invent a date.",
      properties: {
...
- An obituary: handle with CARE. Someone photographs an obituary to show up for a person who is grieving. You NEVER show up for the dead, so the DECEASED IS NEVER A PERSON to return — do not put the deceased in the people array at all. Read the deceased's name only as CONTEXT. The people you return are the named living SURVIVORS (spouse, children, parents, siblings — the grieving relatives the user might know). On each survivor set the event occasion to "loss of <the deceased's name>" (the passing/service date if one is printed, recurring:false). If two or more survivors are named, set ambiguous_multi_person and list each — never auto-pick. If NO survivor is named, return an empty people array (the confirm step will let the user say who they are showing up for). Only use names and relationships literally printed. Keep everything plain and factual; do not add sentiment.
...
- A relative or friend merely MENTIONED in a message ("my mom is visiting", "Eli started school") is a NOTE about the visible person, never their own person record. (An obituary is the exception: the named living survivors ARE returned as people, because showing up for them is the point — but the deceased is never returned.)
...
- If TWO OR MORE distinct people appear (a group chat, a couple on an invite, several named survivors in an obituary), set ambiguous_multi_person:true and list each person separately. NEVER pick one for the user.
