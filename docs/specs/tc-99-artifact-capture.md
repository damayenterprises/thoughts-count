...
- **Obituary — the deceased is NEVER a person to add (David, 2026-08-12):** you never show up *for* the deceased, so the deceased is never a candidate person. Extract the deceased's name ONLY as loss **context** (`occasion:"loss of <name>"`). The candidate people to add/show up for are the **named living survivors** (the bereaved relatives/friends the user actually knows). The confirm step asks **who are you showing up for?** among those survivors; if none are named on the artifact, let the user say/pick who. Never auto-pick a survivor; never surface the deceased as an addable person. Faithful-only (names/relations literally printed), quiet condolence copy, never cheery.
